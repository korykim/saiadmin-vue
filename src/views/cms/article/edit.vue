...
let title = computed(() => {
  return '文章表' + (mode.value == 'add' ? '-新增' : '-编辑')
})
...
// 验证规则
const rules = {
  category_id: [{ required: true, message: '文章分类必需选择' }],
...
const initPage = async () => {
  const resp = await categoryApi.getPageList()
  // 将后端返回的数据转换为TreeSelect所需格式
  treeData.value = formatTreeData(resp.data)
}
